              ...
              <Dialog open={showPatientDialog} onOpenChange={setShowPatientDialog}>
                ...
                <DialogContent data-testid="add-patient-dialog">
                  ...
                  <form onSubmit={handleAddPatient} className="space-y-4">
                    ...
                    <div>
                      <Label htmlFor="patient-doctor">{t('selectDoctor')}</Label>
              ...
              <Dialog open={showAppointmentDialog} onOpenChange={setShowAppointmentDialog}>
                ...
                <DialogContent data-testid="add-appointment-dialog">
                  ...
                  <form onSubmit={handleAddAppointment} className="space-y-4">
                    <div>
                      <Label htmlFor="appt-patient">{t('selectPatient')}</Label>
                    ...
                    <div>
                      <Label htmlFor="appt-doctor">{t('selectDoctor')}</Label>
                  ...
                  <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">{t('doctorName')}</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">{t('totalCost')}</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">{t('totalPaid')}</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">{t('balance')}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                      {patients.map((patient) => (
                        <tr key={patient.id} data-testid={`patient-row-${patient.id}`} className="hover:bg-slate-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{patient.total_cost.toFixed(2)} {t('JOD')}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-emerald-600">{patient.total_paid.toFixed(2)} {t('JOD')}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-orange-600">{patient.balance.toFixed(2)} {t('JOD')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
